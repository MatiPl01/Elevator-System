...
export class ElevatorService {
  ...
  updateFloors(minFloorNum: number, maxFloorNum: number) {
    ...
    for (let floorNum = minFloorNum; floorNum < this._floors.minFloor; floorNum++) {
      newFloors.heights.push(config.defaultsConfig.minPossibleFloorHeight);
    }

    // Add the current floors that are between the new minFloorNum and
    // the new maxFloorNum
    const startIdx = minFloorNum - this._floors.minFloor;
    const endIdx = maxFloorNum - this._floors.maxFloor;
